...
const __filename$1 = url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('hhhan-utils.cjs', document.baseURI).href)));
const __dirname$1 = path.dirname(__filename$1);

function readFileUpwards(file, startDir = __dirname$1, maxLevels = 3) {
  try {
    if (!file) return
...
function RSA(data, publicKey) {
  ...
  const encrypted = crypto$2.publicEncrypt(
    {
...
function SHA1withRSA(data, privateKey) {
  const sign = crypto$2.createSign('SHA1'); // 创建一个签名对象，使用 SHA1 哈希算法
